...
public class TaskService {
    ...
    public void saveTask( Long projectId,Task task){
     ...
     public void deleteTaskById(Long taskId){
        ...
        Project project=task.getProject();
         if (project == null) {
        throw new IllegalArgumentException("Project not found for the task with id: " + taskId);
    }
    ...
    public void updateTaskById(Long id,Task taskDetails){
        ...
        if(taskDetails.getTitle()!=null && taskDetails.getTitle().length()>0 && !task.getTitle().equals(taskDetails.getTitle())){
            task.setTitle(taskDetails.getTitle());
        }
        if(taskDetails.getDescription()!=null && taskDetails.getDescription().length()>0 && !task.getDescription().equals(taskDetails.getDescription())){
